# Automatic Speech Recognition using CNN+GRU Neural Network and CTC Loss Function
...
where the _(nfft + 1)_ in the numerator is added beacuse the FFT coefficient are symmetric, and the +1 ensures that the highest frequency bin is included. Then we have to round down resulting Hertz values to the nearest FFT bin index. The scaling factor is derived from the frequency resolution of the FFT mentioned earlier.   This step is necessary because the filter bank needs to be applied to the magnitude spectrogram, which is computed using the FFT, so the mel frequency points need to be converted to FFT bin numbers.  
